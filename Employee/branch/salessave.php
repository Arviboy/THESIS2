 ...
 if(isset($_POST['submit']))
{
  ...
  $checkiftherearesold = mysqli_query($conn,"SELECT * FROM $branchassign WHERE sackid = '$sackid' AND retail = 0 AND wholesale = 0");
  ...
  $result = mysqli_query($conn,"SELECT * FROM dailysales WHERE branchname = '$branchassign' AND riceid = '$sackid' AND dateofsale = CURDATE()");
    if (mysqli_num_rows($result)){

      $updaterow = mysqli_query($conn,"UPDATE dailysales a, $branchassign b
      ...
      $resultupdateopricetotal = mysqli_query($conn,"UPDATE dailysales d, riceinfo r SET d.oprice = r.originalprice, d.opricetotal = d.sold * r.originalprice WHERE d.branchname = '$branchassign' AND d.riceid = '$sackid' AND d.dateofsale = CURDATE() AND d.riceid = r.id ");
